...
spl2, nleaf2, llvl2, xleaf2, numleaves2 = jz.tree.summarize_leaves.jit(
    xleaf, max_size=32, nleaf=nleaf, num_part=len(posz), ref_fac=8)
il, ir2l2, ispl = jz.knn.build_ilist_recursive.jit(
...
il, ir2l2, ispl = jz.knn.build_ilist_knn.jit(*par, alloc_fac=256, k=k, boxsize=boxsize)
ispl.block_until_ready()

il2, ir2l2, ispl2 = jz.knn.build_ilist_knn.jit(*par, alloc_fac=256, k=k, boxsize=boxsize)

rnn, inn = jz.knn.ilist_knn_search.jit(posz, spl, il, ir2l2, ispl, k=k, boxsize=boxsize)
...
for rfac in 2, 4, 8, 16, 32, 48:
    ...
    if read:
        ...
        il, ir2l, ispl = jz.knn.build_ilist_recursive.jit(
    ...
    # timer.timeit_jit(jz.knn.build_ilist_knn.jit, *par, alloc_fac=222, k=k, sort=False, name="nosort")
    timer.timeit_jit(jz.knn.build_ilist_knn.jit, *par, alloc_fac=222, k=k)

    il,ir2l,ispl = jz.knn.build_ilist_knn.jit(*par, alloc_fac=222, k=k, boxsize=boxsize)
    if read:
    ...
    # timer.timeit_jit(jz.knn.build_ilist_knn.jit, *par, alloc_fac=180, k=k, sort=True, name="sort")
print("")